...
class DenoisingAutoencoderPipeline:
    def __init__(self, data_path, random_state=42, noise_factor=0.5):
    ...
    def _add_noise(self, data):
        ...
        """
        noisy_data = data + self.noise_factor * np.random.normal(loc=0.0, scale=1.0, size=data.shape)
...
class CustomAutoencoderHyperModel(kt.HyperModel):
    ...
    def get_hyperparameters(self):
        ...
        if regularization_type == 'l2':
            regularizer = regularizers.l2(2.419650235738947e-06)
    ...
    def build(self, hp):
        """
        Builds and compile the autoencoder model.
        """
    ...
    def train_autoencoder(self, autoencoder, X_train_reshaped, epochs, batch_size, early_stopping=None, n_splits=5):
        ...
        for train_index, val_index in kf.split(X_train_reshaped):
            ...
            X_train_noisy = self._add_noise(X_train_fold)

            history = autoencoder.fit(X_train_noisy, X_train_fold, 
...
def main():
    ...
    pipeline = DenoisingAutoencoderPipeline(data_path)
    ...
    history = hypermodel.train_autoencoder(autoencoder, X_train_good_reshaped, 15, 16)
    ...
    autoencoder.save('trained-models/3-stds-denoising-autoencoder.keras')
    ...
    with open('results_3stds_denoising_ae.pkl', 'wb') as f:
        pickle.dump(data, f) 
