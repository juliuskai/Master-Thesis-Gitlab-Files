...
class CustomAutoencoderHyperModel(kt.HyperModel):
    ...
    def fit(self, hp, model, X_train_reshaped, X_train_reshaped_original, num_folds=5, *args, **kwargs):
        ...
        for train_index, val_index in kfold.split(X_train_reshaped_original):
            ...
            # Add noise to the training data, but use regular val_data as target data
            X_train_noisy = self._add_noise(X_train_fold)

            history = model.fit(
                X_train_noisy, X_train_fold,
...
def main():
    ...
    tuner.reload()
    helper_methods.save_hyperparameters_with_styled_headers(tuner, output_pdf='denoising-ae-hyperparameters_table_std3.pdf', output_csv='denoising-ae-hyperparameters_std3.csv')
